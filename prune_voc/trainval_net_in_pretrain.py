...
if __name__ == '__main__':
  ...
  output_dir += f"_bs{args.batch_size}_lr{args.lr}_lrds{args.lr_decay_step}_"+\
                f"_lrw{args.lr_warmup}_rs{args.random_seed}"+\
                f"{pretrain_string}"
  ...
  if os.path.exists(os.path.join(output_dir,f'faster_rcnn_{args.session}_{args.max_epochs}_{checkpoint}.pth')):
    print('Skipping training as {} exists'\
          .format(os.path.join(output_dir,f'faster_rcnn_{args.session}_{args.max_epochs}_{checkpoint}.pth')))
    quit()
  ...
  for epoch in range(args.start_epoch, args.max_epochs + 1):
    ...
    if epoch == args.max_epochs:
      save_name = os.path.join(cur_output_dir, 'faster_rcnn_{}_{}_{}.pth'.format(args.session, epoch, step))
      save_ckpt_dict = {
