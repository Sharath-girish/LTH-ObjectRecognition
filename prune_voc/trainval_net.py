...
#lth
  ...
  parser.add_argument('--nr', dest='num_rounds',
                      help='number of rounds for iterative pruning, 2 is for one shot pruning',
                      default=1, type=int)
  ...
  parser.add_argument('--ml', dest='module_list',
                      help='7 character binary string for modules to prune (base_conv|top_conv|rpn_conv|top_fc|rpn_fc|downsample|bn). 1 means module is pruned',
...
if __name__ == '__main__':
  ...
  output_dir += "_layerwise" if args.layerwise else ""
  checkpoint = int(len(roidb)/args.batch_size)-1
  ...
  if run_lth:
    ...
    print(f'Param percentage for pruning: {n_params/n_total*100}%')
  ...
  for cur_round in range(args.num_rounds):
    ...
    if cur_round>=1:
      ...
      if args.layerwise:
        fasterRCNN = lth_pruner.generate_new_mask_layerwise(fasterRCNN, cur_round)
      else:
    ...
    for epoch in range(args.start_epoch, args.max_epochs + 1):
      ...
      for step in range(iters_per_epoch):
        iter_number = iters_per_epoch*(epoch-1)+step
        if args.lr_warmup > iter_number:
          if epoch==args.start_epoch:
            mult = (iter_number+1)/iter_number if iter_number > 0 else 1/args.lr_warmup
            warmup_learning_rate(optimizer, mult)
        if cur_round == 0 and run_lth and iter_number == lth_pruner.late_reset_iter:
